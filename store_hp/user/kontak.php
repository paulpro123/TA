...
if(isset($_POST['send'])){
   ...
   $select_message = $koneksi->prepare("SELECT * FROM `messages` WHERE name = ? AND email = ? AND number = ? AND message = ?");
   ...
   if($select_message->rowCount() > 0){
      ...
      $insert_message = $koneksi->prepare("INSERT INTO `messages`(user_id, name, email, number, message) VALUES(?,?,?,?,?)");
