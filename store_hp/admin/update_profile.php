...
if (isset($_POST['submit'])) {
   ...
   $update_profile_name = $koneksi->prepare("UPDATE `admins` SET name = ? WHERE id = ?");
...
<body>
   ...
   <form action="" method="post">
    ...
    <?php if (isset($fetch_profile) && is_array($fetch_profile)): ?>
        <input type="hidden" name="prev_pass" value="<?= htmlspecialchars($fetch_profile['password']); ?>">
        <input type="text" name="name" value="<?= htmlspecialchars($fetch_profile['name']); ?>" required placeholder="Masukkan nama pengguna Anda" maxlength="20" class="box" oninput="this.value = this.value.replace(/\s/g, '')">
    <?php else: ?>
        <p style="color: red;">Data profil tidak ditemukan.</p>
    <?php endif; ?>
